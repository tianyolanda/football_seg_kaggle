...
def evaluate(model=None, inp_images=None, annotations=None,
    ...
    for inp, ann in tqdm(zip(inp_images, annotations)): # 每张图
        ...
        for cl_i in range(model.n_classes): # 每个类别
    ...
    n_pixels_norm = n_pixels / np.sum(n_pixels)# 该类别的像素数/全像素数
    frequency_weighted_IU = np.sum(cl_wise_score*n_pixels_norm) # 是0-1的数
    ...
    return {
        ...
        "class_wise_pixels_norm": n_pixels_norm
    }
